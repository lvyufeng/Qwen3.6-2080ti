...
class RequestScheduler:
    def __init__(self, *, max_history: int = 128, clock: Callable[[], float] | None = None) -> None:
        ...
        self._running: OrderedDict[str, _RunningRequest] = OrderedDict()
    ...
    def begin_next(self) -> GenerateRequest:
        if self._running:
            raise SchedulerError(f"request is already running: {next(iter(self._running))}")
        return self.begin_next_running()

    def begin_next_running(self) -> GenerateRequest:
        try:
            request = self._pending.popleft()
        except IndexError as exc:
            raise SchedulerError("no pending request to run") from exc
        self._running[request.request_id] = _RunningRequest(request=request, started_at=self.clock())
        return request

    def begin_many(self, limit: int) -> list[GenerateRequest]:
        if limit <= 0:
            return []
        claimed: list[GenerateRequest] = []
        while len(claimed) < limit and self._pending:
            claimed.append(self.begin_next_running())
        return claimed

    def running_request(self, request_id: str | None = None) -> GenerateRequest | None:
        if request_id is not None:
            running = self._running.get(request_id)
            return None if running is None else running.request
        first = next(iter(self._running.values()), None)
        return None if first is None else first.request

    def running_request_ids(self) -> tuple[str, ...]:
        return tuple(self._running.keys())

    def complete_running(self, result: T) -> ScheduledResult[T]:
        request, started_at = self._require_single_running()
        return self.complete_request(request.request_id, result, started_at=started_at)

    def complete_request(self, request_id: str, result: T, *, started_at: float | None = None) -> ScheduledResult[T]:
        running = self._require_running(request_id)
        ...
        scheduled = ScheduledResult(
            ...
            result=result,
            error=None,
            queued_seconds=_elapsed_seconds(running.request.created_at, started_at),
            run_seconds=_elapsed_seconds(started_at, end),
        )
        self._record_terminal(cast(ScheduledResult[object], scheduled))
        self._clear_running(request_id)
        return scheduled

    def fail_running(self, error: BaseException | str) -> ScheduledResult[object]:
        request, started_at = self._require_single_running()
        return self.fail_request(request.request_id, error, started_at=started_at)

    def fail_request(self, request_id: str, error: BaseException | str, *, started_at: float | None = None) -> ScheduledResult[object]:
        running = self._require_running(request_id)
        if started_at is None:
            started_at = running.started_at
        end = self.clock()
    ...
    def snapshot(self) -> SchedulerSnapshot:
        running_ids = self.running_request_ids()
        return SchedulerSnapshot(
            pending=len(self._pending),
            running=running_ids[0] if running_ids else None,
    ...
    def _has_active_request(self, request_id: str) -> bool:
        return any(request.request_id == request_id for request in self._pending) or request_id in self._running
    ...
    def _require_running(self, request_id: str) -> _RunningRequest:
        try:
            return self._running[request_id]
        except KeyError as exc:
            raise SchedulerError(f"request is not running: {request_id}") from exc

    def _require_single_running(self) -> tuple[GenerateRequest, float]:
        if not self._running:
            raise SchedulerError("no running request to complete")
        if len(self._running) != 1:
            raise SchedulerError("expected exactly one running request")
        request_id, running = next(iter(self._running.items()))
        del request_id
        return running.request, running.started_at

    def _clear_running(self, request_id: str) -> None:

...
def test_cli_tp_generate_single_rank(tmp_path: Path, capsys, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "tp_generate_dispatch_moe_native_grouped_dispatch_calls:" in out
...
def test_cli_tp_generate_fast_decode_flag(tmp_path: Path, capsys, monkeypatch: pytest.MonkeyPatch) -> None:
    config = _runtime_config()
    text = config["text_config"]
    text["hidden_size"] = 256
    text["vocab_size"] = 320
...
def test_cli_tp_worker_runs_protocol_loop_without_human_stdout(
    ...
    assert state.fast_decode is False
    assert str(runtime.device) == "cpu"


def test_cli_tp_worker_passes_fast_decode_flag(
    tmp_path: Path,
    capsys,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert out == '{"ok":true}\n'
    assert len(calls) == 1
    state, _runtime = calls[0]
    assert state.fast_decode is True
